...
const Register = () => {
  ...
  // Inside Frontend/src/Pages/Register/Register.jsx
...
useEffect(() => {
  ...
  const getUser = async () => {
    try {
      ...
      // ✅ FIX: Look for user details inside data.data
      if (data && data.data) {
        const userDetails = data.data;

        // ... (The rest of your logic for education and projects remains the same)
        const edu = userDetails.education || [];
        if (edu) {
          edu.forEach((ele) => { ele.id = uuidv4(); });
        }
        if (edu.length === 0) {
          edu.push({ id: uuidv4(), institution: "", degree: "", startDate: "", endDate: "", score: "", description: "" });
        }
        const proj = userDetails.projects || [];
        if (proj) {
        ...
        setForm((prevState) => ({
          ...
          skillsProficientAt: userDetails.skillsProficientAt || [],
          skillsToLearn: userDetails.skillsToLearn || [],
        ...
        console.error("User details object not found in API response's data property.");
        toast.error("Could not fetch your account details.");
      }
    } catch (error) {
