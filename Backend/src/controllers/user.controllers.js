...
export const UserDetails = asyncHandler(async (req, res) => {
  ...
  const user = await User.findOne({ username: username }).populate('badges');
  if (!user) {
...
export const updateNotificationSettings = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, null, "Notification settings updated"));
});

// Rate a user
export const rateUser = asyncHandler(async (req, res) => {
  const { toUserId, bountyId, score, comment } = req.body;

  if (!toUserId || !bountyId || !score) {
    throw new ApiError(400, "toUserId, bountyId, and score are required");
  }
  ...
  // Check if rating already exists for this user and bounty
  const existingRating = await Rating.findOne({ fromUser: req.user._id, toUser: toUserId, bounty: bountyId });
  if (existingRating) {
    throw new ApiError(400, "You have already rated this user for this bounty");
  }
  ...
  // Calculate new average rating for toUser
  ...
  await User.findByIdAndUpdate(toUserId, { rating: averageRating });
...
export const getUserRatings = asyncHandler(async (req, res) => {
  ...
  const ratings = await Rating.find({ toUser: user._id }).populate('fromUser', 'name username').populate('bounty', 'title');
  return res.status(200).json(new ApiResponse(200, ratings, "Ratings fetched successfully"));
});

// Get user analytics
export const getUserAnalytics = asyncHandler(async (req, res) => {
  console.log("******** Inside getUserAnalytics ********");
  const userId = req.user._id;

  // Completed bounties (assuming bounties with status COMPLETED where user is creator or assigned)
  const postedBounties = await Bounty.find({ createdBy: userId });
  const completedBounties = await Bounty.find({ createdBy: userId, status: 'COMPLETED' });
  const assignedBounties = await Bounty.find({ assignedTo: userId, status: 'COMPLETED' });

  // Ratings received
  const ratingsReceived = await Rating.find({ toUser: userId }).sort({ createdAt: -1 });

  // Average rating
  const averageRating = ratingsReceived.length > 0 ? ratingsReceived.reduce((sum, r) => sum + r.score, 0) / ratingsReceived.length : 1.0;

  // Earnings (placeholder, since no payment system yet)
  const earnings = completedBounties.length * 10; // Dummy calculation

  // Skill growth (ratings over time)
  const ratingsOverTime = ratingsReceived.map(r => ({
    date: r.createdAt,
    score: r.score,
  }));
  ...
  const analytics = {
    ...
    totalAssigned: assignedBounties.length,
    averageRating,
    earnings,
    ratingsOverTime,
  };
...
export const getRecommendations = asyncHandler(async (req, res) => {
  ...
  // Get user's proficient skills
  const userSkills = user.skillsProficientAt || [];

  if (userSkills.length === 0) {
    return res.status(200).json(new ApiResponse(200, { recommendedBounties: [], recommendedUsers: [] }, "No skills found, no recommendations"));
  }

  // Recommend bounties that match user's skills
  const recommendedBounties = await Bounty.find({
    tags: { $in: userSkills },
    status: "OPEN",
    createdBy: { $ne: user._id } // Not own bounties
  })
  .populate("createdBy", "name username picture rating")
  .sort({ createdAt: -1 })
  .limit(10);

  // Recommend users with complementary skills (skills to learn that match others' proficient)
  const userToLearn = user.skillsToLearn || [];
  const recommendedUsers = await User.find({
    skillsProficientAt: { $in: userToLearn },
    _id: { $ne: user._id }
  })
  ...
  return res.status(200).json(new ApiResponse(200, { recommendedBounties, recommendedUsers }, "Recommendations fetched successfully"));
});
